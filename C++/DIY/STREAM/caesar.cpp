...
int main(int argc, char* argv[]) //argc - how many arguments there are, argv[] - matrix that contain the argument
{
  int key = 3;
  ...
  for(int i = 1; i < argc; i++) //Process all command-line arguments
    ...
    {
      file_count++;
      if(file_count == 1) //The fist file name
	{
	...
	{
	  cout << "Usage: " << argv[0] << " [-d] infile output" << endl;
	  return 1;
	}
